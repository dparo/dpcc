...
\urlref{https://www.valgrind.org/}{Valgrind} is a very useful tool for C development mainly used for
memory debugging (memory corruptions, invalid writes accces), finding memory leaks, and for
...
highlighted the current custom allocator implementation to be the \textbf{bottleneck} of the compiler. Most
...
This problem is something that should totally be addressed before shipping a compiler to a final user.

Also I wanted to take some snapshots about the running time of the compilar when runned with source code input
of different sizes. Unfortunately since the current implementation of the allocator is \textbf{slow}, such snapshots
