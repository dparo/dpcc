...
\begin{itemize}
    ...
    \item Most of the rest of the grammar is C-inspired, and in fact all control flow statements have
    ...
    \item The precedence of the operators are taken directly from the \urlref{https://en.cppreference.com/w/c/language/operator_precedence}{C precedence table}.
        The only \textbf{modification that DPL does respect to C} is that bitwise operators (\texttt{&, |, \^})
        have higher precedence to the compare operators (\texttt{==, !=, \dots}). Most modern languages
    ...
    \item A \textbf{DPL} program starts either in 2 ways. The first more idiomatic way is to just start
        writing statements directly like dynamic languages do:
        \begin{lstlisting}[language=DPL]
let a = 10;
print(a + 20);
        \end{lstlisting}
...
fn main() {
    ...
    added in the grammar of the language for consistency reason if functions where ever be implemented.
    ...
    \item Code blocks are enclosed in braces `\texttt{\{ \dots \ \}}'. Each code blocks define a new scope where variables can
        be defined.
...
The Ast marks each node with an \texttt{ast\_node\_kind\_t} so that we can disambiguage easier in later
...
Mostly overkill, but allows to generate C code for checking the type of the expressions.
It allows to quickly generate bake the preamble and postamble C code required by the compiler
to be emitted when called with \texttt{`./dpcc c <input>'} mode. The preamble and postamble
are read from this script as a normal C files, and are converted into C arrays, dumped
into a file so that they can finally be embedded at compilation in the final executable.
