...
is self contained and doesn't reach for external assets file and thus can be placed
anywhere in the system and invoked from anywhere in the system.
...
To call the compiler invoke, which will print it's usage help message:

\begin{lstlisting}[language=Bash]
./dpcc
\end{lstlisting}

The \texttt{-o} flag is mostly used to override the location of where output files/stream are generated if any.

\textbf{dpcc} can work in 6 different modes: \textbf{lex, parse, 3ac, c, gcc, run}:

\begin{itemize}
    \item \texttt{./dpcc lex <input> [-o <out>]}: Lex the input and show list of tokens in either stdout or in the given file
    \item \texttt{./dpcc parse <input> [-o <out>]}: Parse the program and produce a text representation of the AST in either stdout or in the given file.
    ...
    \item \texttt{./dpcc c <input> [-o <out>]}: Same as 3AC but also emit preamble and postamble required to produce a semantically valid C program that can be compiled. The output is emitted in either stdout or in the given file
    ...
    \item \texttt{./dpcc run <input>}: Parse, typecheck, emit the C code, compile it and run it in one single command. The executable produced by GCC is outputted in a temp file (\texttt{/tmp}), the temp executable is executed right away and the removed. The \texttt{-o} flag is ignored. This requires GCC to be in the path.
\end{itemize}

\texttt{Lex} and \texttt{parse} mode are mostly used for debugging the produced tokens and the AST. The \texttt{run} mode
is the most convenient mode since it takes care of everything. If the input program is valid and
you call \texttt{dpcc run} you will see the output generated from you \textbf{DPL} program.
